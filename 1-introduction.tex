...
We will develop the necessary analytical methods for evaluating path integral in Chapter~\ref{ch:feynman_kac},
...
This section provides an overview of the thesis.  We will also present some key results, by referencing where the result 
appears later in the thesis, before quoting the resulting.   
The rest of this thesis is laid out as follows: 
In Chapter~\ref{ch:EM_quantization} we formally quantize
...
Initially, we develop the full vector path integral for the EM field partition function~(\ref{eq:Z_4pot}).  
...
Instead of that approach, we split the EM field into two noninteracting scalar polarizations:
the transverse-electric (TE) and transverse-magnetic (TM) polarizations.
...
In Chapter~\ref{ch:feynman_kac} we discuss the analytical methods for solving single-particle path 
...
In the steady-state $(t\rightarrow \infty)$ limit, the solution is given by the following path integral [Eq.~(\ref{eq:f_soln})],
\begin{align}
...
\end{align}
In simple cases, the diffusion equation~(\ref{eq:diffusion_equation}) can be solved directly to find 
an analytical expression for the path integral.  
Most importantly, in Section~\ref{sec:TM boundary condition} we  show that a regularized form of the TM potential [Eq.~(\ref{eq:VsubTM})]
\begin{equation}
...
\begin{align}
  ...
  \right.  
\end{align}
The analytical ensemble-average over all paths interacting with $V\subTM$ has even further smoothed out the singular potential. 
...
In Chapter~\ref{ch:analytical} we use the results from the previous chapter to derive analytical 
...
We also discuss the transition between high and low temperature, and near field limits within the
worldline context.  

In Chapter~\ref{ch:numerical} we discuss the numerical methods for evaluating worldline path integrals.
This will typically involve using Monte Carlo sampling methods for generating paths 
and sampling the starting positions $\vect{x}_0$ and path times $\cT$.  
These methods are discussed for the TE polarization in Section~\ref{sec:TE_Casimir_numerics}.
In Section~\ref{sec:TE_results}, we present the numerical results for the TE polarization,
...
Finally, our findings and directions for future work are summarized in the conclusion.  
