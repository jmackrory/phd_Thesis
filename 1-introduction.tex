...
The fist emphasizes a full vector path integral for the EM field partition function~(\ref{eq:Z_4pot}),  
...
Chapter~\ref{ch:feynman_kac} discusses the analytical methods for solving single-particle path 
...
Most importantly, Section~\ref{sec:TM boundary condition} shows that a regularized form of the TM potential [Eq.~(\ref{eq:VsubTM})]
\begin{equation}
...
The chapter ends by discussing the transition between high and low temperature, and near field limits within the
worldline context.  

Chapter~\ref{ch:numerical} presents the numerical methods for evaluating worldline path integrals.
This typically involves using Monte Carlo sampling methods for generating paths 
...
Chapter~\ref{ch:force} develops general expressions for the force, torque and derivative of the force 
