...
London's perturbation theory assumes that the dipole interact with another instantaneuously, which
...
The complex contour integral can be split into two pieces: the integral over the right semi-cicrcle 
...
contribution to the force~\cite{Sushkov2011}, as opposed to the faster $d^{-2}--d^{-3}$ behaviour expected
...
while the energy scales as $d^{-6}$--$d^{-7}$ for pairs of atoms.  
 ...
 $\omega_T=\kB T/\hbar$.  The different scaling behaviours emerge from differing approximations to the 
...
In that case, helium satisfies the repulsive Casimir criterion and it is energetically favourable
