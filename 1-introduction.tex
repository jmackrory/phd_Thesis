...
well as the advent of new experiments forcing new theoretical methods.  
...
the Casimir effect for various fields, dimensions~\cite{Milton2002}.  While these works discuss the theory, 
there have also been the books in that discuss the state-of-the-art in both theory and experiments
by practitioners in the volumes by Bordag~\etal\cite{Bordag2009}, 
and the collection editted by Dalvit~\etal\cite{Dalvit2011}.  
\todo{Cite Vogel and Welsch.  Also cite Buhmann}
While this thesis emphasizes simple physical applications, the application of Casimir or 
van der Waals forces is important to problems in surface physics and chemistry, are discussed in the texts by 
...
and $\epsilon_0$ is the permittivty of free space.  Throughout this thesis we will refer to these
...
(This interaction can often treated instead with $H_{\text{int}} = -\op{\vect{d}}\cdot \op{\vect{E}}$ where
these are connected by the Power-Zienau transformation~\cite{SteckNotes}.\todo{Find page})
...
\end{figure}
  Fig.~\ref{fig:feynman_CP} represents the perturbation theory computation of 
