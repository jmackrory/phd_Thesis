...
and since in QED each mode carries a certain amount of energy (even in the vacuum).
...
We will then briefly discuss relevant experiments in Sec.~\ref{sec:expt_review}, and expand on our discussion
...
These interatomic forces were described in terms fluctuating, induced dipoles using quantum mechanical perturbation
 theory~\cite{London1930}.  This work leads to an interatomic potential with a characteristic $d^{-6}$ scaling.
While this is similar to the other possible dipole-dipole interactions, these dispersion forces are often the dominant contribution
to intermolecular forces~\cite{Israelchivili2011}.
...
Casimir then showed that neutral, conducting plates would be attracted together~\cite{Casimir1948}.  This arises 
since the total energy in the quantized electromagnetic field is reduced due to the field's interaction
...
from the pair-wise van der Waals interactions of all of the constituent parts~\cite{Dzyaloshinski1961}.  
...
% \end{align}
If the atom is close to the $z=0$ plane, but very far from the other walls, the $x,y$ sinusoids
...
In the case of an atom near a dielectric interface, the Casimir--Polder potential is 
\begin{equation}
...
\todo{Note change for dielectric: $0<\eta(\chi)<1$.}
...
(Our treatment parallels \S 7.2 in Milonni~\cite{Milonni1994}, and Lambrecht\etal\cite{Lambrecht2011}.)
...
\begin{equation}
  E = \frac{\hbar}{16\pi^3 i}\int dk_Tk_T\oint d\xi \, \ln\Delta(\xi).
\end{equation}
...
\end{equation}
following Zhou~\cite{Zhou1995}. The perfect-conductor Casimir can be found by taking $r^{(\zeta)}_i\rightarrow 1$,
...
\todo{Compare to adatom force for adsorbed atomic dipole polarizing another atom.
Note: real atomic dipole inducing another one.  Casimir--Polder is atom interacting with it's own 
fluctuating dipole image.}

\todo{Patch potentials due to variations in work function of metal (different crystalline axes have different work functions),
leads to localized distributions of charge.  Get $1/r$ sort of force background.  Longer-ranged
than Casimir force. }
...
\subsubsection{Distance Regimes}
...
At the other limit, for distances beyond $10 \mu$m, the Casimir effect becomes too weak to detect.  

The typical distance scales can be estimated by considering the dominant 
...
\todo{Cite Sabisky and Anderson for Liquid Helium}
...
account in making theoretical predictions, the surface must also be carefully characterized~\cite{vanZwol}.
...
\cite{Levin2010}.  However, in this example the repulsion is not stable. \comment{citation for stability}
