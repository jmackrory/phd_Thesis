...
interact with electrons in other bodies.  The interacting bodies can be pairs of atoms~\cite{CasimirPolder1948a}, 
...
carried out to measure the forces between atoms and surfaces~\cite{Sukenik1993,Perrault2007,Harber2005}.
...
Casimir forces in arbitrary arrangements of bodies~\cite{Reid2009,Reid2011,Reid2013}.  \todo{Describe 
physics of scattering method}
\todo{Introduce worldline method briefly.}

 while this thesis aims to expand upon the worldline method.  
...
will be discussed further in Sec.~\ref{sec:expt_review}.  
...
allowed modes.  (This interpretation is bolstered by examining the Casimir force between mirrors~\cite{Genet2003,Genet2004}.)
...
Each path is weighted with a phase $e^{iS[x(t)]/\hbar}$ where $S[x(t)]$ is the classical action for the path.
    ...
    While offering an intuitive picture of quantum mechanics, path integrals form 
    a natural basis for quantum field theories, where they offer a relativistically covariant
    ...
    processes~\cite{Kac1949,Durrett1996, Karatzas1991}.  This mathematical work works instead weights each
    path by $e^{-S_{E}[x]}$, where $S_E$ is the Euclidean action for the path.
    ...
    to quantize the electromagnetic field, the worldline method for computing Casimir energies relies 
    upon them.  In addition, we will use the connection between path-integrals and diffusion equations
    to find analytical solutions, and Monte-Carlo sampling of path-integrals is the basis for our numerical
    method.  
    Considering their importance to this thesis, we will 
...
% \end{enumerate}
  ...
  %   Replaces oscillating Gaussian integrals for real, decaying ones.
    ...
    to $x_f$ at time $t_f$ is given by 
    \begin{equation}
      \langle x_f,t_f| x_i, t_0\rangle = \langle x_f| e^{-i\op{H}t/\hbar}|x_i\rangle.
    \end{equation}
    For a particle in a time-independent potential $V(\vect{x})$, the Hamiltonian is 
    \begin{equation}
    ...
    and $N$ resolutions of the momentum identity between
    the pieces
    \begin{align}
    ...
    in $\Delta t$.  
    (In general, it is crucial to carry out all expansions in path-integrals to $\order(\Delta t)$.  
    ...
    At this point, the quantum operators be replaced by their eigenvalues turning the exponential 
    ...
    \end{align}
    where we also used the overlap between position and momentum eigenstates.
    ...
    \begin{align}
      ...
      &= \int \prod_{k=1}^{N-1} d^Dx_k \prod_{k=0}^{N-1} \frac{d^Dp_k }{(2\pi\hbar)^D}
        ...
        &\hspace{6cm}        \times e^{i m (\vect{x}_{n+1}-\vect{x}_n)^2/(2\Delta t)}e^{ -iV(\vect{x}_n)\Delta t/\hbar}\bigg\}\\
        ...
        \exp\left\{\frac{i}{\hbar}\int_{t_i}^{t_f} dt\,\left[ \frac{m}{2} \dot{\vect{x}}^2-V[\vect{x}(t)]\right]\right\},
    \end{align}
    ...
    In this thesis, this type of derivation will be all we require.  For convergence reasons we 
...
We will note similarities to our work in Ch.~\ref{ch:quantization})
...
where now the reflection coefficients are matrices encoding the scattering between modes.
This has been applied for two-body systems such as realistic mirrors~\cite{Lambrecht2006}
and spheres and planes~\cite{Canaguier-Durand2012}.  Instead of reflection
coefficients for a surface, one considers the full scattering matrix for each body, which describes the scattering
between polarizations.  These variants rely on analytical expansions, and while they do offer a general purpose numerical
method, they may be slow to converge if the choice of basis functions poorly corresponds to the actual
geometry required.  

The Johnson group at MIT has developed a better formulation, where instead of decomposing the field 
using basis functions like spherical harmonics,
...
In effect, the surface patches appear to have fluctatuing currents which interact with one another by emitting 
