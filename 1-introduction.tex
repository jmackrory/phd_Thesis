...
and unless carefully removed, can swamp measurements of the Casimir effect. 
...
while the energy scales as $d^{-6}$--$d^{-7}$ for pairs of atoms.  
 ...
 of the interacting media, such as the atomic polarizabilities $\alpha(\omega)$ or the dielectric
 function $\epsilon(\omega)$.  Furthermore at nonzero temperature, there is a thermal wavelength,
 $\omega_T=\kB T/\hbar$.  The different scaling behaviors emerge from differing approximations to the 
integrals in the Lifshitz formula~(\ref{eq:lifshitz}) in various limits.  
 ...
 dipole interaction between the media.   So for the atom-wall interaction, the potential shows a $d^{-3}$ scaling. 
...
as $d^{-4}$ for an atom-wall interaction
 ...
 $\lambda_T=2\pi\hbar c/(\kB T)~50\mu m$.  In this regime the potential falls off more slowly as $E\sim d^{-3}$,
...
\cite{Levin2010}.  However, in this example the repulsion is unstable. 
In fact, it seems that it is impossible to get stable levitation
for similar bodies via Casimir forces alone~\cite{Rahi2010}.
...
The boundary elements are triangles that tessellate the surface, where each triangular patch is 
thought to have a current on it.  
These surface patches interact with one another with the homogeneous Green function.  
