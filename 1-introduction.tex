...
\begin{equation}
  V\subCP = -\sum_{m\ne n,\vect{k},\zeta} \frac{
    \langle g, 0|  H_{\text{int}}|E_n, 1\subkz\rangle\langle E_n, 1\subkz|  H_{\text{int}}|g, 0\rangle}{\hbar(\omega_k+\omega_n)},
\end{equation}
where $|E_n,1\subkz\rangle$ denotes the state with the atom in energy level $n$ and one photon in mode $\vect{k}$,
$|0\rangle$ denotes the vacuum state of the electromagnetic field, and the transition frequency $\omega_n=(E_n-E_g)\hbar$.
...
At this point, the mode functions for the electric field near a perfectly conducting plane can
be substituted in(\S 8.4.3 of Steck~\cite{SteckNotes}, or \S3.12 in Milonni~\cite{Milonni1994}), with 
\begin{equation}
