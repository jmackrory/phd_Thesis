...
It is a natural manifestation of the fact that even in the vacuum state, there is still some
...
One can derive this as a low energy approximation from the fundamental theory of QED~(see Appendix A of \cite{Rahi2009}).  
...
one can approximate this interaction via the electron's linear response.  
...
\todo{Effective interaction: expanding out perturbation theory leads to $\langle j_\mu j_\nu\rangle$
...
\caption[Casimir Energy in terms of fundamental QED processes. ]
 ...
 but still interact with one another via exchanging photons.  The effective interaction of the 
...
As small as this energy is, it is the only attractive energy shift around for neutral atoms.
\todo{Tempererature comparison}
...
\begin{enumerate}
    \item Casimir force per area: $F=\frac{-\pi^2\hbar c}{240d^4}$
  \item Compare to Coulomb force between (pg. 219 of \cite{Milonni1994}) oppositely charged plates.
    $F_{C} = 2\pi\sigma^2$.
    \item Coulomb force for charge densities.  
    \begin{align}
      F = \int d^2xd^2y\,\frac{\sigma_1\sigma_2}{4\pi\epsilon_0[d^2+(x-y)^2]}
    \end{align}
    \item Go to relative-cylindrical coordinates.
    \begin{align}
      F &= -(\int d^2x)2\pi \int_0^y dy\,y\frac{\sigma_1\sigma_2}{4\pi\epsilon_0[d^2+y^2]}\\
      &= -(\int d^2x)2\pi \frac{\sigma^2}{4\pi\epsilon_0}\int_0^y dy\,\frac{y}{[d^2+y^2]}\\
      &= -(\int d^2x)2\pi \frac{\sigma^2}{4\pi\epsilon_0}\int_0^\infty dy\,\frac{y}{d^2+y^2}\\
    \end{align}

    \todo{Size of patch potentials for Lamoreaux}
\end{enumerate}
...
Casimir forces are short ranged forces and decay away quickly --- for example the 
...
\end{equation}
\comment{Need positive operators?}
...
\todo{Note connection to semiclassical scattering approach of Schaden, and Scardiccio}.
\begin{enumerate}
  \item Semi-classical approach to Casimir force via trace formula~\cite{Schaden1998}
  \item Also the optical path-integral \cite{Scardicchio2004, Scardicchio2006}.
\end{enumerate}
