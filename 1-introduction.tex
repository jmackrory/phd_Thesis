...
% A quantum field theory posits that the physical world is described by continuous fields, where the 
% fields are excited in discrete amounts that we interpret as the particles or quanta of the field.  \todo{Cite Schwinger on Why QFT?}
% Interactions between fields are mediated by exchanging quanta of the appropriate field.
% For example, in electromagnetism charged matter 
% such as protons and electrons interact with one another by emitting and reabsorbing photons which are the quanta of the 
% electromagnetic field.  
% (We will distinguish between real particles which can be observed on a detector, and virtual particles
% which can interact with other particles, but are not directly observable).
% Since the electromagnetic field is quantized, 
...
London described these inter-atomic forces in terms of fluctuating, induced dipoles using quantum mechanical perturbation
 theory~\cite{London1930}.  This work leads to an inter-atomic potential with a characteristic $d^{-6}$ scaling.
...
The Casimir energy can be found with an ingenious argument due to van Kampen\etal\cite{vanKampen1968}.
...
Exactly the same style of argument can be used to find the thermal Casimir force, 
...
formula, and can recover all of the limiting behaviours at close distance, far field, and perfect conducting
media, and rarefied media.

\subsection{Physical Interpretation}

\comment{This should go first!  What is it, how big is it, how does it compare to other effects.}
...
% the field.  This framing emphasizes the fields, and the constraints imposed via idealized boundary conditions.
% However, this view obscures the connection to the fundamental physics of electrons interacting with photons.

Casimir's original calculation vividly shows the importance of vacuum fields.  
No mention is made of any interactions, just fields with imposed boundary conditions.  
However, this obscures the physics that went into creating the boundary conditions, and makes it harder
 to connect the Casimir effect to other fundamental interactions.
...
boundary conditions.  Fig.~\ref{fig:electron-effective-interaction} shows a term contributing to the Casimir effect where 
electrons on different bodies interact with one another via the electromagnetic field.  The solid lines should
be understood as the current operators for the electrons bound to a particular, separate media, while the 
wavy lines are the EM green functions describing the photon.  
In that case the closed electron loops should be understood as current-current correlation functions,
$\langle j_\mu j_\nu\rangle$, where under a linear response theory, this correlation function is related to the conductivity
...
and the material functions used in the Casimir effect.  
...
considering metal bodies~\cite{Lamoreaux2011}.  
...
or subtracted off to extract the Casimir effect.  These various electrostatic potentials are random and show a $d^{-1}$ power law
...
and approximating the integral in various limits. 
 ...
 resonant wavelengths for the bodies $d\ll \omega_A/c,\omega_M/c$, the exponential factor $e^{-2pd\xi/c}$
 is unity for all relevant frequencies, and each frequency contributes equally.  In essence, the interaction is an instantaneous 
 dipole interaction between the media.  For example the atom-wall potential shows a $d^{-3}$ scaling. 

 The retarded or Casimir-Polder regime occurs when the atoms are much further than a resonant wavelength 
...
and the functions can be approximated with their static limit.
 ...
 In this far-field regime, the potential typically decays more quickly,
as $d^{-4}$ for an atom-wall interaction
...
the primary sources of error in these experiments.
 In addition, the optical properties of the surface must also be carefully characterized~\cite{vanZwol2011}.
...
involving atomic beams, cavity QED, and Bose-Einstein condensates.  
...
% In this case it was ``easy'' to vary the distance of the atoms from the surface from $1\mu m$ up to 10 $\mu m$ to observe
...
quantum devices for computation and simulation of quantum systems~\cite{Kimble2008}.  
Atoms are an attractive platform for a number of reasons:
Each atom of the same species is identical (unlike some other candidate systems); atoms have 
well-defined transitions that can be used to control their motion, internal state and coupling; and they 
have internal states that are long-lived, which is important in storing information.
...
One direction that has been pursued is the so-called atom-chip~\cite{Folman2000,Schneider2003,Salem2010},
...
scale~\cite{Dimopoulos2003, Bezerra2011}.  The new physics must be short-ranged, and is typically modeled as 
...
field induced by the presence of bodies~\cite{Rahi2009}.  The energy is renormalized via $\mathbb{M}^{-1}_\infty$,
...
These variants rely on scattering between mode functions suited to analytical expansions,
...
Green function.  Since the method assumes piece-wise, homogeneous media and enforces EM boundary
conditions, it is the relatively simple homogeneous EM Green function that appears in these expressions.
The surface currents can also be integrated over, leaving a functional determinant analogous to Eq.~(\ref{eq:scattering})
where now the matrix elements $\mathbb{M}_{ab}$ describe the coupling between surface-patches.

This is particularly adaptable as the basis patches can be chosen to cover an arbitary body easily---
in particular a triangular mesh is the most flexible.
...
% and $\mathbf{M}$ is a matrix where the entries encode the scattering between various surface current 
% elements via the homogeneous EM green function,  
% \begin{equation}
...
where the Casimir force is repulsive due to geometric effects~\cite{Levin2010}, altough the repulsion
...
force is an essential component of the trap.  
...
however, these methods were developed for free-space interactions at high energy.
...
In these non-trivial geometries the worldline method also examines the failure of the proximity force approximation.
More recent work has focused on computing the stress-energy tensor~\cite{Schafer2012, Schafer2016},
with a view to exploring how the Casimir energy violates certain energy conditions (violations of which are required for exotic physics).
...
for the semiclassical approach to the Casimir force by Schaden\etal\cite{Schaden1998} that evaluates 
the Casimir energy by summing over all periodic orbits of light.  Both of these approximate techniques
rely on a path integral expression for the Casimir energy that approximates electromagnetism as a
...
\end{equation}
where the labels over positions have been labelled with indices and the matrix $A$ represents the differential
operator.  
...
\end{equation}
The trace can be evaluted by introducing a $D$-dimensional auxiliary Hilbert space, where 
...
In addition, the final integral $\int d\tau_0 = \beta\hbar c$, since $\tau_0\in[0,\beta\hbar c]$.  
...
The distance dependence can also be read off.  The typical extent of a Brownian motion of path-time
...
\begin{align}
  \mathcal{F}-\mathcal{F}_0 =& -\kB T {\sum_{n=0}^\infty}'\int \frac{d\cT}{(2\pi\cT)^{(D-1)/2}\cT} \int d\vect{x}_0
...
As a result, the $\cT$ dependence differs, which implies that the distance dependence differs at nonzero temperature.
