...
can be replaced by their average value of a $1/2$, since they will be quickly oscillating.   
...
In this case, the energy is renormalized by subtracting the energy when the bodies are moved arbitrarily far away.  
...
\end{align}
where the primed sum gives $1/2$ the weight to the $n=0$ term, and all functions of frequency are evaluated at $s_n.$ 
...
of the systems.  Since the mode functions are changed in a global, nonlinear fashion, the sum over frequencies
also changes in a nonlinear fashion as more bodies are added.  
...
Derivations similar to the argument principle used in Sec.~\ref{sec:lifshitz} can be applied to describe the scattering 
...
effect involve one loop, since the electron emits and reabsorbed a photon.
...
quantum lead to a non-trivial effective action.}, 
