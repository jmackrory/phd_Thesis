...
For example, the electric field Green function responding to electric sources at imaginary frequency is 
$\Gamma^{EE}(\vect{x},\vect{x'})=e^{-\kappa|\vect{x}-\vect{x'}|}/(4\pi|\vect{x}-\vect{x'}|)$.  Similarly simple expressions
exist for the other combinations of electric/magnetic responses to electric or magnetic sources.
The surface currents can also be integrated over, leaving a functional determinant analogous to Eq.~(\ref{eq:scattering})
where now the matrix elements $\mathbb{M}_{ab}$ describe the coupling between surface-patches induced
by the Green function.  (An expanded derivation is given in App.~\ref{app:scattering}.)

%\subsubsection{Sketch of derivation}
...
This method and alternative choice of basis is particularly adaptable as the basis patches can 
be chosen to cover an arbitary body easily---
...
\begin{align}
  H &:= \int d^3x\,(\Pi\partial_t\phi -  L)= \int d^3x\,\bigg[\frac{\Pi^2}{2} + \frac{1}{2}(\nabla\phi)^2 +V(\vect{x},t)\phi^2\bigg].  
\end{align}
...
only needing an ensemble of simple Brownian motions, and Monte Carlo sampling.
