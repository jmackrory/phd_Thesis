...
Each path is weighted with a phase $e^{iS[x(t)]/\hbar}$ where $S[x(t)]$ is the classical action for the path.
    ...
    that underly the Standard Model of particle physics~\cite{Brownn1994,Srednicki2008}.
...
% \end{enumerate}
  ...
  %   Replaces oscillating Gaussian integrals for real, decaying ones.
    ...
    and exploiting the equivalent of the Ito rule $dx^2=dt$~\cite{deWitte1965,Kleinert2012,Grosche1994}.)
...
in $D$ space-time dimensions, in a potential $V$.  In exactly the same fashion as Sec.~\ref{sec:feynman_path_integral},
this can be converted into a single-particle path integral.

    
Worldline path integral at zero temperature 
  \begin{equation}
    F = -\frac{\hbar c}{(2\pi)^{D/2}}\int \frac{d\cT}{\cT^{1+D/2}} \dlangle e^{-\cT\langle V\rangle} - 1\drangle,
  \end{equation}
