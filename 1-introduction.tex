...
\begin{equation}
  ...
  \frac{\omega_{n0}\langle E_0|d_i|E_n\rangle\langle E_n| d_j|E_0\rangle}{\omega_{n0}^2-\omega^2},
\end{equation}
...
If we assume the atom's distance from the surface $d$ is larger than the atom's dominant wavelength's
or $d/c\gg \omega_{n0}$, then we can use fact the atom's static (zero frequency) polarizability 
is given by 
\begin{equation}
  \lim_{\omega\rightarrow 0}\alpha(\omega) = \sum_n
  \frac{|\langle E_0|\vect{d}|E_n\rangle|^2}{3\omega_{n0}},
\end{equation}
...
or mode functions for a perfectly conducting box(\S3.12 in Milonni~\cite{Milonni1994}), with 
\begin{equation}
  \vect{f}\subkz(\vect{r}) = \sqrt{\frac{2}{V}}\left(\hat{\varepsilon}  \right)
\end{equation}  


In order to extract a finite energy shift it is essential to renormalize the energy by subtracting
off the constant, divergent energy associated with an atom infinitely far away from the surface.   
...
\todo{Fix this fucking mess of a Casimir--Polder calculation}
...
The energy in the EM field in vacuum is 
\begin{equation}
...
~\cite{vanKampan1968,Schaden1998}.
\todo{Sums over $k_x,k_y,\omega$, with $k_z$ given by }
The mode sum can be recast as a contour integral over a function whose poles occur at the allowed
frequencies, with residue $\omega_k$:  
\begin{equation}
  E = \oint dz \frac{\hbar z}{2(2\pi i)} \frac{\Delta'(z)}{\Delta(z)},
\end{equation}
...
\begin{equation}
  E = \frac{\hbar}{4\pi i}\oint dz \, \ln\Delta(z).
\end{equation}
\todo{Remaing integrals over transverse $k_T$?}
\begin{itemize}
