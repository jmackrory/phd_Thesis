...
where the ghost degrees of freedom cancelled~\cite{Bordag1999}.  However any attempt to move away from spherical
...
integral terms should be interpreted as a metric.   In this case, $\epsilon(x)$ would also mean the diffusion constant of the paths 
 vary throughoutt space, which complicates path construction.  
...
that was used for the Dirichlet scalar, and full vector path interal. 
The resulting partition function is 
\begin{equation}
...
It is a straightforward matter to develop the TE worldline path-integral.
 In the same fashion as the Dirichlet scalar method in Sec.~\ref{sec:dirichlet_worldline}, 
...
If we further approximate the material responses by their dc values, then the integral over frequency
is Gaussian.  After carrying out the frequency integral, the result is the zero temperature TE worldline (\ref{eq:TE_worldline}).
