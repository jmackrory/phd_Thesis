...
% The classical Lagrangian for electromagnetism in vacuum is 
% \begin{equation}
%   L=-\frac{1}{4}\int d^3\vect{r}\, F_{\mu\nu}F^{\mu\nu} =-\frac{1}{2}\int d^3\vect{r}\, (|\vect{E}|^2-|\vect{B}|^2).
% \end{equation}
...
The classical Lagrangian for electromagnetism in non-dispersive media is
\begin{align}
...
% \begin{equation}
% Z  = \prod_{i=1}^N\int d\phi_id\vect A_i \delta(\vect A_N-\vect{A}_0)\delta(\phi_N-\phi_0)
% \langle \phi_i,\vect A_i|e^{-\Delta \beta\hat H}|\phi_{i-1},\vect A_{i-1}\rangle,
% \end{equation}
% where we have defined $A^\mu_N=A^\mu_0=A^\mu$.
%   Now we note that each $A_i$ corresponds to a particular temperature range, 
% so we can write $A_i = A_{\beta+\Delta \beta}$ and $A_{i-1}=A_\beta$.  
...
% % \langle \phi_{\beta+\Delta \beta},\vect A_{\beta+\Delta \beta}|\Pi_{0,\beta} \vect{\Pi}_\beta\rangle 
% % \langle \Pi_{0,\beta}\vect{\Pi}_\beta|\exp[-\Delta \beta \hat{H}]|\phi_\beta,\vect A_\beta\rangle\\
% & = \int d\vect{\Pi}_\beta\,\delta(\nabla\cdot\vect{\Pi}_\beta)
% \langle \vect A_{\beta+\Delta \beta}| \vect{\Pi}_\beta\rangle
% \langle \vect{\Pi}_\beta|\exp[-\Delta \beta \hat{H}]|\vect A_\beta\rangle.
% \end{align}
...
\langle \phi_{\beta+\Delta \beta}\vect A_{\beta+\Delta \beta}| e^{-\Delta \beta \hat{H}}|\phi_\beta, \vect A_\beta\rangle
=
% \int\! d\vect{\Pi}_\beta \int\! D\varphi_\beta\exp\bigg[ \int d^3r \,
% -\frac{i}{\hbar}\varphi_\beta(\nabla\cdot\vect{\Pi}_\beta) +\frac{i}{\hbar}\left(\vect A_{\beta+\Delta \beta}
% -\vect A_{\beta}\right)\cdot\vect{\Pi}_\beta  -\Delta \beta\left( \ frac{\vect{\Pi}_\beta^2}{2\epsilon_0\epsr(\vect{r})}
...
\begin{multline}
\langle \phi_{\beta+\Delta \beta}\vect A_{\beta+\Delta \beta}| e^{-\Delta \beta \hat{H}}|\phi_\beta, \vect A_\beta\rangle\\
%  \int\! d\vect{\Pi}_\beta \exp\left[ -\Delta \beta\int d^3r \frac{\vect{\Pi}_\beta^2}{2\epsilon_0\epsr(\vect{r})} 
...
% \begin{equation}
%   Z = \int D^4A\, \exp\left[-\frac{1}{2}\int d^4r\,
%   \epsr(\vect{r})\left(\partial_4\vect A-\nabla A_4\right)^2+(\nabla\times\vect A)^2\right].
% \end{equation}
The field path integral can be written 
 \begin{equation}
 Z = \int D\vect{A} DA_4\, e^{-S\subE[\vect{A},A_4]},\label{eq:Z_potential}
 \end{equation}
...
% In order to proceed it is necessary to fix the gauge in the path integral.
...
% to account for the gauge-fixing, since the gauge transformation is non-linear in the fields.  
  For electromagnetism, since the gauge transformation is independent of the potentials $A^\mu$,
...
Of these gauges, the generalized Lorenz gauge is best suited to the four-potential path integral~\ref{eq:Z_potential}.  
%(In later work on the scalar path integrals in planar geometries, Coulomb gauge can be used to naturally.)
In this case gauge function is
\begin{equation}
...
\begin{align}
  ...
  +\epsr\sqrt{\mur}\partial_4(A_4+\partial_4\alpha)\nonumber\\
&=G[A^\nu]+\bigg[\frac{1}{\epsr\sqrt{\mur}}\nabla\cdot\epsr\nabla+\epsr\sqrt{\mur}\partial_4^2\bigg]\alpha.
\end{align}
The Euclidean action (\ref{eq:euclidean_action}) after gauge-fixing, canceling terms, and integrating by parts, becomes 
\begin{align}
  S\subE &=\frac{1}{2}\int d^3r d\tau\,\bigg[ A_4L^{(4)}A_4   +A_iL^{(V)}_{ij}A_j \bigg],\label{eq:S_pot_vec}
\end{align}
...
integral terms should be interpreted as a metric.  It would also mean the diffusion constant of the paths 
...
% operators like $\partial_x\epsilon\partial_x$ will lead to a non-Gaussian probability density, which is
...
% where $V$ arises from commuting space-dependent dielectric functions past the derivatives.
...
\begin{align}
  \tilde{L}^{(4)} &= -\epsr\partial_\tau^2-\frac{1}{\sqrt{\epsr}}\partial_k\epsr\partial_k\frac{1}{\sqrt{\epsr}}.\label{eq:scalar_operator}\\
  \tilde{L}^{(V)}_{ij}&=
