...
The new parameter $\tau$ is proportional to the thermal deBroglie wavelength.  The resulting partition function is
 \begin{equation}
 Z = \int D\vect{A} DA_4\, \det[\epsr]^{3/2}e^{-S\subE[\vect{A},A_4]},\label{eq:Z_potential}
 \end{equation}
where the Euclidean action is 
\begin{equation}\label{eq:euclidean_action}
S\subE[\vect{A},A_4] = \frac{1}{2} \int d^3r\,\int_0^{\beta\hbar c}d\tau\, \bigg(
...
\begin{align}
  ...
  &\hspace{3.5cm}-2\big[\exp(-M_\text{g}\cT) - \exp(-M\sup0\cT)\big]\bigg)
\end{align}
...
The EM field can be split into two non-interacting polarizations for media that only vary in one Cartesian dimension in one direction.
...
The material properties $\epsr(z)$ and $\mur(z)$ also only vary in this direction.  
...
In generalized Coulomb gauge $\nabla\cdot\eps\vect{A}=0$, and using Maxwell's equations, the magnetic
field $B$ can also be written in terms of this scalar and $|\vect{B}|^2=|\nabla\phi|^2$.
