...
is attended by noise, either as decoherence terms in the master equation or as noise terms in the Langevin equations for the operators~\cite{Gardiner2004}}.
...
\propto   \det[\epsr(\vect{r})]^{3/2}\exp \left\{ -\frac{\epsilon_0\Delta \beta}{\hbar^2}
  \int d^3r\,\bigg[ \frac{\epsr(\vect{r})}{2}(\partial_\beta \vect A_\beta - \nabla\varphi_\beta)^2
  +\frac{1}{2}\epsilon_0c^2(\nabla\times \vect A_\beta)^2\bigg]\right\},
\end{multline}
where the functional determinant $|\epsr|^{3/2}$ comes from the momentum integrals.  
...
The field path integral can be written in terms of the Euclidean action,
 \begin{equation}
...
physical state.  %The gauge-fixing  is done by enforcing a delta-function constraint on the gauge degree of freedom.  
