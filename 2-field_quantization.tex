...
$\beta = i\tau$.
  At each step there is also a $\delta$-function restricting the allowed momentum states.
...
% \end{align}
Since $\Pi_0$ is constrained to vanish, we can carry out that integral immediately, and focus on the other
...
\propto   \det[\epsr(\vect{r})]^{3/2}\exp \left\{ -\frac{\epsilon_0d\beta}{2\hbar^2}
  \int d^3r\,\bigg[ \epsr(\vect{r})(\partial_\beta \vect A_\beta - \nabla\varphi_\beta)^2
...
\begin{equation}
\int D \vect{A} D\varphi |\epsr(\vect{r})|^{3/2} = \prod_{i=1}^N\prod_{r_k}\int d\vect{A}(\vect{r}_k,\beta)
