...
\begin{itemize}
\item We aim to solve the diffusion equation given by 
\begin{equation}
  \partial_t f = \frac{1}{2}\nabla^2 f  - [V+\lambda]f +g,
\end{equation}
where $f=f(\vect{x},t)$, $V=V(\vect{x},t)$, and $g=g(\vect{x},t)$.  
\item Interpretation.  Statistics: $f$ is probability distribution, this is Fokker-Planck
    equation describing evolution of probability distribution, driven by a source. 
    Undergoing killing rate given by $V(x)$. 
    Quantum: evolution of wave function for quantum particle in a potential $V$.  
\item Can derive path integral using techniques from quantum. Solve by introducing Hilbert space 
    Commutation relation
    \begin{gather}
      [\op{x}_i,\op{p}_j]=i\delta_{ij}
    \end{gather}
    Spatial representation of operators
    \begin{gather}
      \langle \vect{x}|\op{x}_i|\psi\rangle = x_i\psi(\vect{x})\\
      \langle \vect{x}|\op{p}_i|\psi\rangle = -i\partial_i\psi(\vect{x})
    \end{gather}
    Resolutions of the identity
    \begin{gather}
      I_x = \int d\vect{x}|\vect{x}\langle \rangle \vect{x}|, \\
      I_p = \int \frac{d\vect{p}}{(2\pi)^D}|\vect{p}\langle \rangle \vect{p}|
    \end{gather}
\item Can write diffusion equation as operator equation:
\begin{equation}
  \partial_t f = \frac{1}{2}\nabla^2 f  - [V+\lambda]f +g,
\end{equation}
