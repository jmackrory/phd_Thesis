...
worldline path integral in planar geometries.
The sharp dielectric interfaces are modeled by step functions, and the analytical solutions are given by  Eqs.~(\ref{eq:Feynman-Kac TE one step}) and
...
% These analytical solutions (such as for Dirichlet )may also be useful in accelerating numerical methods relying 
...
In this particular case, the vector Wiener increments are defined as $\Delta \vect{W}_j := \vect{x}_{N-j-1}-\vect{x}_{N-j}$.
...
%  \sum_{k=0}^{j} \Delta \vect{W}_k = \vect{x}_{N-j}-\vect{x}_N\rightarrow 
  \vect{x}_j = \vect{x}_N+\sum_{k=0}^{N-j-1} \Delta \vect{W}_k= \vect{x}_N+\vect{W}_{N-j},
\end{equation}
where the Wiener process is defined as $\vect{W}_j:=\sum_{k=0}^{j-1} \Delta \vect{W}_k$.
...
the sum over potentials $\sum_jV(\vect{x}_j)\Delta t$ can be written as an integral.  The path integral can then be written as
\begin{align}
...
step is small relative to the scale over which the potential changes,
 the contribution from each step should be well approximated by the interaction with a single plane. 
...
% in Section~17.10 of \citet{Steck}.)
...
In this case, we will not invert the Laplace transform, since in Chapter~\ref{ch:analytical} we will convert
the relevant Casimir energies to exploit these analytical expression. 
...
\begin{equation}
  V\subTM(x-d) = \frac{1}{2}\big[(\partial_x\log\sqrt{\epsilon})^2-\partial_x^2\log\sqrt{\epsilon}\big].
  \label{eq:VsubTM}
\end{equation}
...
\begin{equation}
  V\subTM(x-d,\Xi) \sim \frac{1}{2}\big[\Xi^2\delta^2(x-d)-\Xi \delta'(x-d)\big],
\end{equation}
...
\begin{equation}
  V\subTM\supa(x-d,\Xi) = \frac{\Xi^2}{2a^2}\Theta(x-d)\Theta(d+a-x)-\frac{\Xi}{2a}[\delta(x-d)-\delta(x-d-a)\big],
  \label{eq:VTM_reg}
\end{equation}
...
We will now show that the regularized TM potential~(\ref{eq:VTM_reg}) imposes an effective boundary condition at the interface. 
...
%    Eq.~(\ref{eq:M d1}) fixes $B$, which we can use in to Eq.~(\ref{eq:M d2}), so that 
    ...
    then Eq.~(\ref{eq:M d1}) implies that $B\sim\order(a)$, since $B/a$ is proportional to $\partial_x f_1$.  
...
\begin{equation}
  f\subTMBC=\int_0^\infty dt\, e^{-\lambda t}\frac{e^{-c^2/(2t)}}{\sqrt{2\pi t}} \dlangle e^{-\int_0^T dt\, V\subTM(x-d,\Xi)}\drangle.
\end{equation}
...
\begin{equation}
  0 = \frac{1}{2} \partial_x^2 f\subTMBC - [V\subTM(x-d,\Xi)+\lambda]f\subTMBC + \delta(x-c),
\end{equation}
...
\begin{align}
  \dlangle e^{-\int_0^t dt'\, V\subTM(x-d,\Xi)}\drangle 
  &=\left\{ \begin{array}{lc} 
...
The path integral
    \begin{equation}
      f\subTMone = \int_0^\infty dt\, e^{-\lambda t}\frac{1}{\sqrt{2\pi t}}\bigdlangle e^{-\int_0^t dt'\, [V\subTM(x-d,\Xi) + \chi\Theta(x-d)]}\bigdrangle ,
    \end{equation}
    is the steady-state solution to 
    \begin{equation}
      \partial_t f\subTMone = \frac{1}{2}\partial_x^2f\subTMone -[V\subTM(x-d,\Xi) + \chi\Theta(x-d)+\lambda]f\subTMone +\delta(x). 
    \end{equation}
...
\begin{equation}
  V = \chi_1\Theta(d_1-x)+\chi_2\Theta(x-d_2)+V\subTM(d_1-x,\Xi_1)+V\subTM(x-d_2,\Xi_2).
\end{equation}
