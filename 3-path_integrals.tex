...
Given the prevalence of the Dirichlet worldline path integral discussed in Sec.~\ref{sec:dirichlet_worldline},
...
\begin{equation}
  ...
  \label{eq:VTM_reg}
\end{equation}
as sketched in Fig.~\ref{fig:TMpot}.
The only singularities present are the $\delta$-functions from the second derivative.
The potential is still singular, and it is difficult reliabily simulate for a single path as $a\rightarrow 0$.
...
%    Our goal is to derive the effective conditions imposed on the solutions just outside the surface.
    ...
    external solutions are assumed to be independent of $a$.% As far as this 
    % calculation is concerned, these left and right solutions  not need are unspecified constants. 
...
\end{equation}
The Gaussian factor is the probabiity density for a 1D Brownian bridge between $x=0$ and $c$ in time $t$.
