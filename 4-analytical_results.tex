...
The worldline Casimir energies can be put in the right form to use the solutions from Ch.~\ref{ch:feynman_kac}.  
...
$\delta$-function, and the limit of an arbitrarily small partile can be taken at the end of the computations.  
...
\end{align}
These expressions should be renormalized by subtracting off the equivalent expressions with a constant dielectric of permittitivity $\epsr(\rA)$.
...
This is directly useful in rewriting worldline path integrals in terms of their Laplace transforms, 
...
to the relevant diffusion equation~(\ref{eq:diffusion_eq}), as discussed in Ch.~\ref{ch:feynman_kac}.
...
\frac{\sqrt{\lambda+s(1+\chi)}-\sqrt{\lambda+s}}{\sqrt{\lambda+s(1+\chi)}+\sqrt{\lambda+s}}.
\end{equation}
This can be transformed by changing variable from $\lambda$ to $v:=\sqrt{\lambda/s+1}$, 
\begin{equation}
  ...
  \frac{\sqrt{v^2+\chi}-v}{\sqrt{v^2+\chi}+v}.
\end{equation}
After change of variables from $s$ to $t:=\sqrt{8d^2 s}\,v$, and swapping the $t$ and $v$ integrals, the result is
\begin{align}
...
=\frac{1}{8^{D/2-1}d^D}\int_1^\infty dv\,v^{-D} (v^2-1)^{(D-1)/2-\alpha} 
  \frac{\sqrt{v^2+\chi}-v}{\sqrt{v^2+\chi}+v}\int_0^\infty dt\, t^{D-1}e^{-t}.\label{eq:J}
\end{align}
The $t$ integral has the value, $\Gamma[D-1]$.  
...
\begin{align}
  \cV_{\nu,\alpha}:=\,&\frac{1}{\Gamma[\alpha]\Gamma[\nu-\alpha+1/2]}\int_0^\infty d\lambda\,\lambda^{\nu-1/2-\alpha}
...
and the integral can be transformed in the same manner as in Sec.~\ref{sec:TE_CP}, 
\begin{align}
  \cV_{\nu,\alpha}% =&\frac{(-1)\sqrt{2\pi}}{\Gamma[\alpha]\Gamma[z-\alpha+1/2]}\int_0^\infty d\lambda\,\lambda^{z-1/2-\alpha}
=\,&-\frac{\sqrt{\pi}\Gamma[2\nu ]}{8^\nu d^{2\nu }\Gamma[\alpha]\Gamma[\nu -\alpha+1/2]}\int_1^\infty dv\,\frac{1}{v^{2\nu }}(v^2-1)^{\nu -1/2-\alpha}
...
-\big(f\supTE_{2}(\vect{x}_0)-f_{1}\sup0\big)\right].
  ...
  The integration variable $\lambda$ is transformed to $v:=\sqrt{\lambda/s+1}$, with the result % [or $\lambda=s(v^2-1)$]
  \begin{align}
  ...
  &\times\frac{r\supTE_1r\supTE_2e^{-2\sqrt{2s}vd}}{(1-r\supTE_1r\supTE_2 e^{-2\sqrt{2s}vd})}
\left(2d + \frac{\sqrt{2}}{\sqrt{s}\sqrt{v^2+\chi_1}}+\frac{\sqrt{2}}{\sqrt{s}{v^2+\chi_2}}\right),
  ...
  \begin{equation}
    r\supTE_i = \frac{v-\sqrt{v^2+\chi_i}}{v+\sqrt{v^2+\chi_i}}.
  \end{equation}
  Next, the $s$ integral is transformed $s$ to $t=\sqrt{2s}$, then the $t$ and $v$ integrals are swapped, and  
  finally the values of $\alpha=1/2$ and $D=4$ are used. The result of those maniupulations is
  \begin{align}
     \frac{E\subTE-E\sup0}{A}
    &= -\frac{\hbar c}{8\pi^{2}}\int_0^\infty dt \,t^{D-1}  \int_1^\infty dv\, (v^2-1)\nonumber\\
    &\times\frac{r\supTE_1r\supTE_2e^{-2tvd}}{(1-r\supTE_1r\supTE_2 e^{-2tvd})}
...
\begin{align}
  \frac{d r\supTE_i}{dv} &= \frac{d}{dv}\frac{v-\sqrt{v^2+\chi_i}}{v+\sqrt{v^2+\chi_i}}
    ...
    = \frac{-2r\supTE_i}{\sqrt{v^2+\chi_i}}\\
    \frac{d}{dv}\log[1-r\supTE_1r\supTE_2 e^{-2 t v d}] & = 
    \frac{r\supTE_1r\supTE_2 e^{-2 t v d}}{1-r\supTE_1r\supTE_2 e^{-2 t v d}}\left( 2 t d+\frac{2}{\sqrt{v^2+\chi_1}}
...
\begin{align}
  ...
  \frac{d}{dv}\bigg[\frac{1}{t}\log(1-r\supTE_1r\supTE_2 e^{-2tvd})\bigg]\nonumber\\
  &= \frac{\hbar c}{4\pi^2}\int_0^\infty dt \,t^{2}  \int_1^\infty dv\, v \log(1-r\supTE_1r\supTE_2 e^{-2tvd}),
\end{align}
...
\begin{equation}
  \gamma\subTE(\chi_1,\chi_2):=-\frac{180}{\pi^4}\int_0^\infty dt\,t^{2} \int_1^\infty dv\,\log[1-r\supTE_1r\supTE_2 e^{-2tvd}].
  \label{eq:gammaTE}
\end{equation}
The efficiency is the ratio of the TE Casimir energy between the dielectrics to the total Casimir energy
between perfectly-conductoring plates at the same distance.
...
This requires evaluating integrals over $x_0$ and combining the resuts. 
...
\end{equation}
The same transformations as in Sec.~\ref{eq:TE_energy} can be employed to find the TM Casimir energy.
After changing integration variables from $\lambda$ to $v:=\sqrt{\lambda/s+1}$, the result is 
  \begin{align}
...
&= -\frac{\hbar cA}{(2\pi)^{D/2}}\int_0^\infty ds\,\frac{s^{(D-2)/2}}{\Gamma(\alpha)}
  \int_1^\infty dv\,\frac{(v^2-1)^{(D-1)/2-\alpha}}{\Gamma[(D+1)/2-\alpha]}\nonumber\\
  &\hspace{0.5cm}
\times\frac{\sqrt{\pi} \,r\supTM_1r\supTM_2e^{-2\sqrt{2s}vd}}{(1-r\supTM_1r\supTM_2 e^{-2\sqrt{2s}vd})}
...
where the reflection coefficients are now given by 
\begin{equation}
  r\supTM_i =  \frac{e^{2\Xi_i}v-\sqrt{v^2+\chi_i}}{e^{2\Xi_i}v+\sqrt{v+\chi_i}}.
\end{equation}
...
%   \int_1^\infty dv\,\frac{(v^2-1)^{(D-1)/2-\alpha}}{\Gamma[(D+1)/2-\alpha]}\nonumber\\
%   &\hspace{0.5cm}
% \times\frac{\sqrt{\pi} \,r\supTM_1r\supTM_2e^{-2tvd}}{(1-r\supTM_1r\supTM_2 e^{-2tvd})}
% %\nonumber\\  &\hspace{0.5cm}\times
% \bigg[2d
...
%  \bigg]
  ...
  &\hspace{0.5cm}
\times\frac{\,r\supTM_1r\supTM_2e^{-2tvd}}{(1-r\supTM_1r\supTM_2 e^{-2tvd})}
%\nonumber\\  &\hspace{0.5cm}\times
\bigg[2d
  -\sum_{i=1}^2\frac{2e^{2\Xi_i}\chi_i}{t\sqrt{v^2+\chi_i}[v^2\,e^{4\Xi_i}-v^2-\chi_i]}
 \bigg].
  \end{align}
Once again, an integration by parts with respect to $v$ will put the energy in standard form. The following
...
After integrating by parts, the TM Casimir energy is 
  ...
  \int_1^\infty dv\,\log[1-r\supTM_1r\supTM_2 e^{-2tvd}].
  \end{align}
\comment{need factor of $4$ - the following assumes it has been found and fixed}.
...
\begin{equation}
  \gamma\subTM(\chi_1,\chi_2):=-\frac{180}{\pi^4}\int_0^\infty dt\,t^{2} \int_1^\infty dv\,\log[1-r\supTM_1r\supTM_2 e^{-2tvd}].
  \label{eq:gammaTM}
\end{equation}
...
The Casimir--Polder energies can be derived by the same reasoning used in Sec.~\ref{sec:casimir-polder_worldline},
...
theorem when using results from Ch.~\ref{ch:feynman_kac} to analytically evaluate the finite-temperature Casimir energy.
...
As noted at the end of Sec.~\ref{sec:nonzero_temp_path_integral}, the zero-temperature, far-field limit can
...
However, this may be superceded by the frequency responses of the atom or medium, as indicated by 
