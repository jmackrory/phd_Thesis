...
% As discussed in \citep[Section~ 4.7]{Jackson1998}, the electrostatic energy for a system depends on the 
...
% The energy for an atom in the EM field can be found by considering the change in the energy 
...
% = &\int_0^\infty \frac{dT}{T^{1+D/2}}\frac{1}{\Gamma[a]}\int_0^\infty ds s^{a-1} 
% \dlangle e^{-s(\chi \int_0^T dt \Theta(x-d) +1)}\drangle \\
% =&\frac{1}{\Gamma[a]}\int_0^\infty \frac{dT}{T^{1+D/2-a}}\int_0^\infty ds s^{a-1} e^{-s T}
...
\end{align}
In the last line $\cT^{n/2}$ was factored out to act as the normalization for a $n$-dimensional Brownian bridge, 
...
\begin{align}
  J %&=2\int_0^\infty ds\, s^{a-1}s^{(D-1)/2-a+1-1/2}\int_1^\infty dv\, (p^2-1)^{(D-1)/2-a}e^{-\sqrt{8 d^2s}p}
...
\begin{align}
  J % =4\int_1^\infty dp\, (p^2-1)^{(D-1)/2-a} \int_0^\infty dt\, \frac{2t}{8d^2p^2}\left(\frac{t^2}{8d^2p^2}\right)^{D/2-1}e^{-t}
  % \frac{\sqrt{p^2+\chi}-p}{\sqrt{p^2+\chi}+p}\\
...
% \frac{\sqrt{\pi}}{\Gamma[a]\Gamma\left[(D+1)/2-a\right]}\frac{\Gamma[D]}{2^{3D/2-2}d^D}\int_1^\infty dp\,p^{-D} (p^2-1)^{(D-1)/2-a} 
%   \frac{\sqrt{p^2+\chi}-p}{\sqrt{p^2+\chi}+p}\\
% =\,&-\frac{\hbar c\alpha_0}{2^{2D-2} \pi^{D/2}\epsilon_0d^D}
% \frac{\sqrt{\pi}6}{(\sqrt{\pi}/2}\int_1^\infty dp\,p^{-D} (p^2-1)^{(D-1)/2-a} 
%   \frac{\sqrt{p^2+\chi}-p}{\sqrt{p^2+\chi}+p}\\
...
% \begin{align}
%   \cV_{\nu,a}=\,&\frac{\sqrt{\pi}}{\Gamma[a]\Gamma[(\nu+1)/2-a]}\int_0^\infty d\lambda\,\lambda^{(\nu-1)/2-a}
%   \int_0^\infty ds\,s^{a-1}  \frac{e^{-2\sqrt{2(\lambda+s)}|d|}}{\sqrt{\lambda+s}} r\subTM\\
% %
% % =\,&\frac{1}{\Gamma[a]\Gamma[(\nu+1)/2-a]}\int_0^\infty ds\,s^{a-1} \int_1^\infty dp\,2sp s^{(\nu-1)/2-a}(p^2-1)^{(\nu-1)/2-a}
% %  \frac{e^{-\sqrt{8sd^2}p}}{\sqrt{s}p} % r\subTM\\
% % =\,&\frac{2}{\Gamma[a]\Gamma[(\nu+1)/2-a]}\int_0^\infty ds\,s^{(\nu-2)/2} \int_1^\infty dp\,(p^2-1)^{(\nu-1)/2-a}
% %  e^{-\sqrt{8sd^2}p}r\subTM\\
% =\,&\frac{2}{\Gamma[a]\Gamma[(\nu+1)/2-a]}\int_0^\infty dt\,
...
% r\subTM\\
%
% =&\frac{(-1)\sqrt{2\pi}}{\Gamma[a]\Gamma[z-a+1/2]}\int_0^\infty d\lambda\,\lambda^{z-1/2-a}
%\end{align}
\begin{align}
...
\begin{align}
\cV_{4,3/2} %=&-\frac{\sqrt{\pi}\Gamma(4)}{8^{2}d^{4}\frac{\sqrt{\pi}}{2}\Gamma(1)}\int_1^\infty dp\,\frac{1}{p^{4}}(p^2-1)^{2-1/2-3/2}
  % \frac{ pe^{2\Xi}-\sqrt{p^2+\chi}} {p e^{2\Xi}+\sqrt{p^2+\chi}}.
...
Evaluating the energy requires integrating the solutions of $\vect{x}_0$ and combining the results. 
...
After changing integration variables from $\lambda$ to $p:=\sqrt{\lambda/s+1}$, the result is 
  \begin{align}
  E\subTM-E\sup0 % &= -\frac{\hbar cA}{2(2\pi)^{D/2}}\int_0^\infty ds\,\frac{s^{a-1}}{\Gamma(a)}
...
be used, with the result
  \begin{align}
  E\subTM-E\sup0 
%   &= -\frac{\hbar cA}{(2\pi)^{D/2}}\int_0^\infty dt\,\frac{t^{D-1}}{2^{(D-2)/2}\Gamma(a)}
%   \int_1^\infty dp\,\frac{(p^2-1)^{(D-1)/2-a}}{\Gamma[(D+1)/2-a]}\nonumber\\
%   &\hspace{0.5cm}
...
%  Phys. Rev. A, \textbf{70},042901,(2004)} (which Dan cites for thermal Casimir-Polder calculations.~\citep{Babb2004})
...
\begin{align}
V\supTM\subCP-V_0%=\,& -k_BT{\sum_n}'\frac{ s_n\alpha(i s_n)}{4\pi\epsilon_0c}\int_1^\infty dp\,\left(\frac{ s_n^2}{c^2}  - \frac{2 s_n^2p^2}{c^2}\right)e^{-2p s_n d/c}\frac{\sqrt{p^2+\chi}-p\,e^{2\Xi}}{\sqrt{p^2+\chi}+p e^{2\Xi}} \\
