...
\begin{equation}
  ...
  \biggdrangle_{\vect{x}(t),\vect{x}(0)=\rA}.
\end{equation}
Each term $\cV_{z,\alpha}$ can in turn be transformed using the combination of the Laplace-Mellin theorem
...
&\hspace{1cm}  -\bigg(\frac{1}{\sqrt{\langle \epsra\rangle}}-\frac{1}{\sqrt{\epsra(\vect{x}_0)}}\bigg)
  ...
  The Casimir energy can be recast using the inverse moment theorem and the Laplace-Mellin theorem,  
  \begin{align}
...
-\big(f\supTE_{2}(\vect{x}_0)-f_{1}\sup0\big)\right].
  ...
  First, change variable from $\lambda$ to $v:=\sqrt{\lambda/s+1}$, with the result % [or $\lambda=s(v^2-1)$]
  \begin{align}
...
-\big(f\supTM_{2}(\vect{x}_0)-f_{1}\sup0\big)\right]\nonumber\\
 ...
 \bigg],
\end{align}
where there is a factor of the transverse area $A$, and the reflection coefficients are given by, 
\begin{equation}
...
\bigg[2d
 ...
 \bigg].
  \end{align}
Once again, an integration by parts with respect to $v$ will put the energy in Lifshitz form. The following
...
The presence of the worldline can be used to estimate the relevant frequencies and dominant parts of the $p$ integral.  
