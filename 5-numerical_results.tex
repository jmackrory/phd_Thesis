...
The numerics for the TM polarization are more complicated than in the TE case, since there is an 
...
\begin{figure}
  \centering
 \includegraphics[width=0.8\textwidth]{fig/numerics/eff_TE_atom_wall}
...
\begin{figure}
  \centering
  \includegraphics[width=0.8\textwidth]{fig/numerics/eff_TE_2wall}
...
averages run over discrete Brownian bridges (which have been analytically averaged over continuous sub-paths between points on the 
...
&  \hspace{0.5cm}+ \dfrac{e^{-(x_{k+1}-c_k x_k)^2/(2c_k \Delta\cT)}}{\sqrt{2\pi c_k \Delta\cT}}  \label{eq:PNC}\\
P\supTM_{\text{C},k+1}(x_{k+1}) &=\dfrac{\sech \Xi}{\sqrt{2\pi c_k \Delta\cT}}e^{-(x_{k+1}-c_k x_k)^2/(2c_k \Delta\cT)}.\label{eq:PC}
\end{align}
...
If the crossing branch is chosen [Eq.~(\ref{eq:PC})], then a deviate is sampled from the truncated Gaussian distribution.
...
Let us consider a generic worldline path integral involving pinned Brownian motions in path time $\cT$ with starting point $\vect{x}_0$:
\begin{equation}
...
% The probability distributions for $\bar{x}_m$ and $\Delta x_m$ can be decoupled and are given by 
% \begin{align}
