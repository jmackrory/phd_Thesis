...
\begin{equation}
  P_x(x;d_0):= \frac{2}{3d}\left\{ \begin{array}{cc}
    1  & |x|<d_0\\
    \frac{d^4_0}{|x|^4} & |x|>d_0
  \end{array}
\right.
\end{equation}
in a one-dimensional geometry, embeeded in a four-dimensional space-time.
  This reasoning can be easily extended to higher dimensional problems,
where in that case an sphere of radius $d_0$ should bound all of the interfaces between the bodies.
...
\begin{figure}
 ...
 \includegraphics[width=0.8\textwidth]{fig/temp/eff_TE_atom_wall}
  ...
  {Planar Casimir--Polder TE energy as function of susceptibility $\chi$.  
    Plots the ratio of Casimir--Polder energy for an
    atom-dielectric interface and an atom--perfect-conductor interface, $\eta(\chi)$.
...
\end{align}
This ensemble-averaged solution is plotted in Fig.~\ref{fig:  }, 
...
two only appear in the strong-coupling limit right at the suface.
...
The numerical properties of the solution can be examing by considering the following simplified path integral
which includes only a single TM potential a distance $d$ away,
\begin{equation}
...
In this case, the analytical solution is known, and we can examine how the numerical answers scale with the number 
of increments.  
...
while a few rare paths return very large values.  
