...
The numerics for the TM polarization are more complicated than the TE case, since there is an 
...
grows as the path resolution $N$ is increased.  Those numerical fluctuations can be tempered by changing the methods
used to construct the paths.  We introduce two methods.  The first is the ``TM-Gaussian'' approach is covered in section~\ref{sec:TM_Gaussian}
which includes the TM potential in constructing the path increments.  However, this also shows the problem
of growing statistical errors,  so in section~\ref{sec:birth_death} we develop the ``birth-death'' method,
where paths branch, or are truncated based on their accumulated weight.  This effectively changes 
the path creation to sample more from important regions of the integral.  
...
Otherwise, if the no-crossing branch is chosen, 
