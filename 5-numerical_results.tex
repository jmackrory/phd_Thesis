...
As the path length $N$ increases for fixed $\chi$, the error shows two different scaling behaviors.  
...
\subsection{Scaling of Averaged TM Potential with Path Length $N$}
...
\end{align}
The normalization constant can be evaluated in two parts $\cN_k=\cN_k^{\text{C}}+\cN_k^{\text{NC}}$, 
 with the result
\begin{align}
...
This may be down to the very small $N$ used in this simulation ($N=200$), which would lead
