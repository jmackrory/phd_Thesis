...
In the weak-coupling limit $\chi/N\ll 1$, the error comes from underestimating the sojourn time.
In the strong-coupling limit $\chi/N\gg 1$, the error comes from underestimating the first path-time
that a path touches the surface.
...
%   \item Rely on importance sampling to determine most important regions of $x_0$, $T$ to sample from.
  \item For time: % For TE loops, integrand is zero before first touching time.  
    % So use $P(\cT,\cT_0) = 2\cT_0^2\Theta(\cT-\cT_0)/\cT^3$.
