...
We presennt our work on the TM polarization as one way of numerically approaching the limit
...
\begin{equation}
  P_{\text{exp-\cT}}(\cT;\cT_0,s) = \frac{\cT_0^{s-1}}{\Gamma[s-1] \cT^s}e^{-\cT_0/\cT}\label{eq:expT},
\end{equation}
...
\begin{equation}
  I=\int d\vect{x} f(\vect{x})P(\vect{x}),
\end{equation}
where $P$ is the coupled Gaussian probability density and $f$ is some function of the path.  
Instead of just taking $P$ as the probability density, let us consider using $Q=P(\vect{x})|_{x_k=d}$,
where one point is fixed.  The resulting Monte Carlo samples would be given by
\begin{equation}
  I = \dlangle \int d\vect{x}_k \frac{P(\vect{x})}{Q(\vect{x})} f(\vect{x})\drangle.
\end{equation}
The ratio of the probability densities is 
\begin{align}
  \frac{P}{Q} &= \frac{\exp\left[-\frac{(\vect{x}_{k-1}-\vect{x}_k)^2}{2\Delta \cT}
...
{\exp\left[-\frac{(\vect{x}_{k-1}-\vect{d})^2}{2\Delta \cT}
  ...
  &= \exp\left[-\frac{(\vect{x}_{k}-\bar{\vect{x}})^2}{\Delta \cT}
...
\begin{equation}
  \cN =\frac{1}{[2\pi k \Delta \cT]^{D/2}[2\pi (N-k)\Delta \cT]^{D/2}}
    e^{-(\vect{x}_0-\vect{d})^2/(2k\Delta \cT)}e^{-(\vect{x}_0-\vect{d})^2/[2(N-k)\Delta \cT]}
\end{equation}
...
Then combine with $P/Q$, and normalization for using $exp-T$
