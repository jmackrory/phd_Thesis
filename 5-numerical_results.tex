...
The simplest method for evaluating derivatives is the finite difference method.  It has the great
virtue of simplicity.  Unfortunately, it behaves poorly at higher order derivatives, and the accuracy
of the results depends sensitively on the step size chosen.  
...
as $\partial_\theta\dlangle f(x)\drangle = \int dx f(x)\partial_\theta P(x,\theta) = \dlangle f(x)\partial_\theta\log P(x,\theta)\drangle$.
The original sampling method can be used to evaluate this derivative.  So a function and its derivatives
can be evaluated from a single stochastic simulation.}
a direct integration by parts approach, followed by method with partial-averaging
along the Brownian motion~\cite{Chen2007}. 
