...
Let's start completing the square at $y_{N-1}$, and introduce a variance $\sigma^2_{N-1}=1$.
...
% If we repeat the completing the square again for $y_{N-1}$ we get
% \begin{equation}
% \frac{(y_{N-1}-(x_{N+1}-x_1))^2}{2(2)}+\frac{(y_{N-1}-y_{N-2})^2}{2} = \frac{1}{2(2/3)}\left(y_{N-1} - \frac{(x_{N+1}-x_1)+2y_{N-2}}{3}\right)^2 + \frac{(y_{N-2}-y_{N})^2}{2(3)}
% \end{equation}
...
\begin{equation}
  x_k = x_0 + \frac{x_N-x_0}{N-k+1}+c_kx_{k-1} + \sqrt{c_k\Delta\cT}z_k
\end{equation}
\comment{double check counting?}

\comment{Jacobian factor of $N$ from multiplying $c_j$ together.}


% So after $k$ recursions, we expect to have 
% \begin{align}
% \frac{(y_{N-k+1}-\Delta x)^2}{2\sigma_k^2}+\frac{(y_{N-k+1}-y_{N-k})^2}{2} 
% = \frac{k+1}{2k}\left(y_{N-k+1} - \frac{(x_{N+1}-x_1)+k y_{N-k}}{k+1}\right)^2 + \frac{(y_{N-k}-\Delta x)^2}{2(\sigma_k^2+1)}
% \end{align}
% For a walk from with $N$ intermediate steps from $x_1$ to $x_{N+1}$, we can do this $N-1$ times.  The end result is:
% \begin{align}
% Y = \sum_{k=2}^{N} \frac{1}{2u_{k}}\left(y_k - u_k y_{k-1} - v_{k}\right)^2 + \frac{(x_{N+1}-x_1)^2}{2N}
% \end{align}
% where $u_k = \frac{N-k+1}{N-k+2}$, and $v_k = \frac{(x_{N+1}-x_1)}{N-k+2}$.  This can be turned into an algorithm as follows: 
% Old
% \begin{align}
% Y = \sum_{k=1}^{N-1} \frac{1}{2u_{k}}\left(y_k - u_k y_{k-1} - v_{k}\right)^2 + \frac{(x_{N+1}-x_1)^2}{2N}
% \end{align}
% where $u_k = \frac{N-k}{N-k+1}$, and $v_k = \frac{(x_{N+1}-x_1)}{N-k+1}$.  This can be turned into an algorithm as follows: 

\begin{enumerate}
\item Sample $N-1$ standard normal deviates, $z_k$, with $k\in\{2,\ldots,N\}$.
\item Invert the transformation of Gaussians to write: 
  \begin{equation}
    z_k = \frac{(y_k - u_k y_{k-1}-v_k)}{\sqrt{u_k}}\rightarrow  y_k = u_ky_{k-1} +v_k + \sqrt{u_k}z_k
  \end{equation}
\item Start the recursion at $k=2$, with $y_1 = 0$, and work forwards to find each $y_k$ in turn.  
\item Add on $x_1$ at the end to get the loop that starts at $x_1$ and ends at $x_{N+1}$.  
\item Remember to include the factor of $\frac{1}{\sqrt{N}}\exp\left(-\frac{\Delta x^2}{2N}\right)$ for normalization.
\end{enumerate}




Gies\etal introduced the v-loop 
Since Gies\etal were considering Casimir energy applications, it makes more sense to consider centered 
paths, where the average position $\langle x\rangle$ is subtracted from the path.  
However in Casimir--Polder (or stress-energy) applications it is better to consider paths emanating from
a single point $\vect{x}_0$.  
...
An analysis of the systematic error due to 
