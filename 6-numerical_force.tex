...
\comment{should have general way of computing gradients }
...
\begin{figure}
  ...
  \caption[Numerically computed TE force]{(Preliminary data) Numerically computed TE force for pinning (squares), occupation (circles)and finite difference (squares)methods.
    ...
    The finite difference had the finite difference step $\delta=d/\sqrt{N}$.
    Simulations had $N=10^4$ points per path, with $10^8$ trajectories.}
  \label{fig:force}
\end{figure}
In Figure~\ref{fig:force}, we plot the numerically calculated force on two dielectric slabs of equal dielectric
...
and evaluate the dielectric path averages using the trapezoidal method discussed in Sec.~\ref{sec:trapezoid}.
...
uniform strata, each of which is sampled from uniformly.  In the simulations in Figure~\ref{fig:force},
