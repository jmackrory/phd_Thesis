...
The correction from the starting path is given by 
\begin{equation}  
  P_{\text{pin-corr}} % = \frac{P_{\text{pinned}}}{P_{\text{free}}}
  % =\frac{ e^{-(d-\sqrt{T}B_{j+1})^2/(2\Delta T)-(d-\sqrt{T}B_{j-1})^2/(2\Delta T)  } }
  % { e^{-(\sqrt{T}B_{j}-\sqrt{T}B_{j+1})^2/(2\Delta T)-(\sqrt{T}B_{j}-\sqrt{T}B_{j-1})^2/(2\Delta T)  }}
  = 
...
We can capture this dependence by sampling times from 
\begin{equation}
  P_C(\cT;B_j,d,N) \propto T^{-a}\exp\bigg[-Nd^2\left(\frac{1}{\sqrt{\cT}}-\frac{B_j}{d}\right)^2\bigg],
\end{equation}
where $a$ is to be determined for convenience. 
The probability distribution can be converted to an exponential distribution via some variable changes.
In particular, if we define 
\begin{equation}
  s = Nd^2\left(\frac{1}{\sqrt{\cT}}-B_j\right)^2,
\end{equation}
and use 
\begin{equation}
  \frac{ds}{d\cT} = Nd^22\frac{-1}{\cT^{3/2}}\left(\frac{1}{\sqrt{\cT}}-B_j\right)
\end{equation}
\begin{equation}
  P_C(s;B_j,d,N) \propto T^{-a}\exp\bigg[-Nd^2\left(\frac{1}{\sqrt{\cT}}-\frac{B_j}{d}\right)^2\bigg],
\end{equation}
...
\begin{align}
  C^{(S)}_{ij} =& \frac{\hbar c}{2(2\pi)^{D/2}}\biggdlangle \int \frac{d\cT}{\cT^{1+D/2}}
  \frac{N^2e^{-N^2d^2/[2 \Delta(N-\Delta)\cT]}}{\sqrt{2\pi \cT \Delta(N-\Delta)}}\nonumber\\
  &\times e^{-[d-\sqrt{\cT}B_\Delta][d-\sqrt{\cT}(B_{\Delta+1}+B_{\Delta-1}-B_\Delta)]/(T/N)}
   \I[1]0\I[2]0 g_{0,0}
   \biggdrangle_{\vect{x}(\cT);\sigma_2(\vect{x}_{j+\Delta}-\vect{R}_2)=0}\\
=& \frac{\hbar c N}{2(2\pi)^{D/2}}\biggdlangle \int \frac{d\cT}{\cT^{1+D/2}}
\bigg(2\cT^{3/2}\sqrt{\frac{\pi}{Nd^2}}\bigg)\bigg(\sqrt{\frac{Nd^2}{\pi}}e^{-Nd^2(B_\Delta/d-1/\sqrt{\cT})^2}\bigg)
  \frac{N e^{-N^2d^2/[2 \Delta(N-\Delta)\cT]}}{\sqrt{2\pi \cT \Delta(N-\Delta)}}\nonumber\\
  &\times 
  e^{-N[d/\sqrt{\cT}-B_\Delta](B_{\Delta+1}+B_{\Delta-1}-2B_\Delta)}
   \I[1]0\I[2]0 g_{0,0}
   \biggdrangle_{\vect{x}(\cT);\sigma_2(\vect{x}_{j+\Delta}-\vect{R}_2)=0},
\end{align}
