...
sample from the combined Hermite-Gaussian.  
\begin{align}
...
\begin{equation}
  P:= \partial_0^n G = \frac{1}{\sqrt{2\pi\sigma^2}\sqrt{2\pi(T_1+T_2)}} (2\sigma^2)^{-n/2}H_n\left(\frac{x_0-\mu}{\sqrt{2\sigma^2}} \right)
  \exp\left[-\frac{(x_0-\mu)^2}{2\sigma^2} - \frac{(x_1-x_2)^2}{2(T_1+T_2)}\right],
\end{equation}
where 
\begin{gather}
  \sigma^2:= \frac{T_1T_2}{(T_1+T_2)}\\
  \mu := \frac{x_1T_2+T_1x_2}{T_1+T_2}.
\end{gather}
...
\begin{gather}
  ...
  \mu := \frac{x_1+x_2}{2}.
\end{gather}
In order to sample $x_1, x_2$ one can sample from both the combined Hermite-Gaussian for $mu=(x_1+x_2)/2$,
and the difference $\Delta x=x_2-x_1$.  
The raw positions can be found by inverting the relations,
\begin{align}
  \mu &= z+x_1\\
\end{align}
\comment{Solve for $x_1, x_2$ in terms of deviates}
